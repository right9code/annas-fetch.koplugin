...
local GITHUB_REPO = "right9code/annas-fetch"
...
function Ota.installUpdate(zip_filepath, plugin_base_path)
    ...
    os.execute("rm -rf plugins/annas-fetch")
    logger.info("Annas:Ota.installUpdate - Trying to clean up old plugin files: 'plugins/annas-fetch'")
...
function Ota.startUpdateProcess(plugin_path_from_main)
    ...
    if not string.match(plugin_path_from_main, "plugins/annas-fetch/?$") then
        local err_msg = string.format(T("Unsupported plugin path for OTA update: %s. Only 'plugins/annas-fetch' is supported."), plugin_path_from_main)
