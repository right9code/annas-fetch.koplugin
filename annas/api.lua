...
function Api.login(email, password)
    ...
    if not rpc_url then
        result.error = T("The Anna's Archive server address (URL) is not set. Please configure it in the Anna's Archive plugin settings.")
...
function Api.getRecommendedBooks(user_id, user_key)
    ...
    if not url then
        ...
        return { error = T("Anna's Archive server URL not configured.") }
    end
...
function Api.getBookDetails(user_id, user_key, book_id, book_hash)
    ...
    if not url then
        ...
        return { error = T("Anna's Archive server URL not configured or book identifiers missing.") }
    end
